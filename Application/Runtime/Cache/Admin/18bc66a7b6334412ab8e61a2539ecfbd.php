...
<title>订单列表 - 订单管理 - Healthtrade后台</title>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
							...
							<td><?php echo ($v["id"]); ?></td>
							<td class="text-l"><a href="/index.php/Admin/Order/detail/id/<?php echo ($v["id"]); ?>"><?php echo ($v["order_num"]); ?></a></td>
							<td class="text-l" ><center><?php echo ($v["money"]); ?></center></td>
							<td><font color="blue"><?php echo ($v["address_id"]); ?></font></td>
							<td><font color="red"><b><?php echo ($v["user_id"]); ?></b></font></td>
							<td class="td-status"><span class="label label-success radius"><?php echo (date("Y-m-d",$v["goods_create_time"])); ?></span></td>
							<td class="f-14 td-manage">
								<a style="text-decoration:none" class="ml-5" href="/index.php/Admin/Order/edit/id/<?php echo ($v["id"]); ?>" title="商品编辑"><i class="Hui-iconfont">&#xe6df;编辑</i></a>
...
function goods_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/index.php/Admin/Order/del/id/'+id,
			dataType: 'json',
