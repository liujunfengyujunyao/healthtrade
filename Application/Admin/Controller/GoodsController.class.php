...
class GoodsController extends Controller{
	...
	public function index(){
		...
		$data=$model -> alias('t1') -> field('t1.*,t2.cate_name') -> join('left join he_category t2 on t1.cate_id = t2.id')-> select();
		// dump($data);die;
		//总记录数
	...
	public function detail(){
		$id=I('get.id');
		$data=D('Goods')->alias('t1')->field('t1.*,t2.cate_name')->join('left join he_category t2 on t1.cate_id = t2.id')->where(['t1.id'=>$id])->find();
		//分配数据到模版
		$this->assign('data',$data);
		$this->display();
	}
